...
export XUNFENG_REPO="https://github.com/xinhaiyucheng/xunfeng.git"
    ...
    case "$lsb_dist" in
        ...
        fedora|centos|redhat|oraclelinux|photon)
            ...
            if [ "$lsb_dist" = "fedora" ] && [ "$dist_version" -ge "22" ]; then
                ...
                (
                    set -x
                    $sh_c 'sleep 3; yum -y -q install epel-release curl wget unzip gcc git libffi-devel python2-devel openssl-devel libpcap newt.x86_64 supervisor ncurses-devel ncurses make gcc-c++ automake autoconf libtool'
                )
            fi
